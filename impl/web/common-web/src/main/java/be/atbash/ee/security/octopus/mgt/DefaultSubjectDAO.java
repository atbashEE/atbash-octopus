...
public class DefaultSubjectDAO {
     ...
     * @see DefaultSessionStorageEvaluator
     */
    protected boolean isSessionStorageEnabled(WebSubject subject) {
        return sessionStorageEvaluator.isSessionStorageEnabled(subject);
    }
     ...
     */
    public WebSubject save(WebSubject subject) {
        // FIXME We need also a read.
     ...
     */
    protected void saveToSession(WebSubject subject) {
     ...
     */
    protected void mergePrincipals(WebSubject subject) {
        ...
        */
        ...
        if (session == null) {
            if (!OctopusCollectionUtils.isEmpty(currentPrincipals)) {
            ...
            if (OctopusCollectionUtils.isEmpty(currentPrincipals)) {
                if (!OctopusCollectionUtils.isEmpty(existingPrincipals)) {
     ...
     * @param subject the Subject for which principals will potentially be merged into the Subject's session.
     */
    protected void mergeAuthenticationState(WebSubject subject) {
     ...
     */
    public void delete(WebSubject subject) {

...
public class LinkedinOAuth2CallbackProcessorTest {
    ...
    @InjectMocks
    private LinkedinOAuth2CallbackProcessor processor;
    ...
    @Test
    public void processCallback_failedLinkedAuthentication() throws IOException, ExecutionException, InterruptedException {
        ...
        assertThat(logger.getLoggingEvents()).hasSize(1);
        assertThat(logger.getLoggingEvents().get(0).getMessage()).isEqualTo("LinkedIn informs us that no valid credentials are supplied or that consent is not given");

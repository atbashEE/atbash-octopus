...
public class GithubOAuth2CallbackProcessorTest {
    ...
    @InjectMocks
    private GithubOAuth2CallbackProcessor processor;

    @Captor
    private ArgumentCaptor<String> redirectStringCaptor;

    private TestLogger logger = TestLoggerFactory.getTestLogger(GithubOAuth2CallbackProcessor.class);
    ...
    @Test
    public void processCallback_failedGithubAuthentication() throws IOException, ExecutionException, InterruptedException {
        ...
        assertThat(logger.getLoggingEvents()).hasSize(1);
        assertThat(logger.getLoggingEvents().get(0).getMessage()).isEqualTo("Github informs us that no valid credentials are supplied or that consent is not given");

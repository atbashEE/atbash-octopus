...
public class WebSubjectCallable<V> implements Callable<V> {
    ...
    // FIXME Parent class with common code from SubjectCallable and WebSubjectCallable
    public WebSubjectCallable(WebSubject subject, Callable<V> delegate) {
        this(new WebSubjectThreadState(subject), delegate);
    }

    protected WebSubjectCallable(ThreadState threadState, Callable<V> delegate) {
        if (threadState == null) {

...
@ProviderOrder(-15)
@ApplicationScoped
public class SystemAccountAuthenticationInfoProvider extends AuthenticationInfoProvider {
    @Override
    protected AuthenticationInfo getAuthenticationInfo(AuthenticationToken token) {
        if (token instanceof SystemAccountAuthenticationToken) {
            SystemAccountAuthenticationToken systemAccountAuthenticationToken = (SystemAccountAuthenticationToken) token;

            String identifier = systemAccountAuthenticationToken.getPrincipal().toString();

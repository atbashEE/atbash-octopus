 ...
 */
package be.atbash.ee.security.octopus.sso.core.rest.reflect;
...
public final class Property implements AnnotatedElement {
    ...
    Property(Bean<?> declaringBean, String name, Method readMethod, Method writeMethod) {
        ...
        this.annotations = Bean.optimizeMap(annotationsMap);
        this.declaredAnnotations = Bean.optimizeMap(declaredAnnotationsMap);
    }
    ...
    public boolean isReadable() {
        return Bean.isPublic(readMethod);
    }
    ...
    public boolean isWritable() {
        return Bean.isPublic(writeMethod);
    }
        ...
        while (current != null) {
            for (Field field : current.getDeclaredFields()) {
                if (field.getName().equals(name)
                        ...
                        && !Bean.isStatic(field)
                        && !field.isSynthetic()
                        && (!Bean.isPrivate(field) || field.getDeclaringClass() == type)) {
                    return field;
                }
            }

...
public class OctopusSSOLogoutURLProcessor implements LogoutURLProcessor {
    ...
    public String postProcessLogoutUrl(String logoutURL, LogoutParameters parameters) {
        if (parameters.isSingleLogout()) {
            OctopusSSOToken octopusSSOToken = parameters.getPrincipalCollection().oneByType(OctopusSSOToken.class);
            return logoutURLCreator.createLogoutURL(logoutURL, octopusSSOToken.getAccessToken());
        } else {
            return logoutURL;
        }
    }

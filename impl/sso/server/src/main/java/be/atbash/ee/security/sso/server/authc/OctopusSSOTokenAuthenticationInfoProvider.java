...
public class OctopusSSOTokenAuthenticationInfoProvider extends AuthenticationInfoProvider {
    ...
    protected AuthenticationInfo getAuthenticationInfo(AuthenticationToken token) {
        if (token instanceof OctopusSSOToken) {
            ...
            builder.userName(ssoToken.getUserName())
                    .principalId(ssoToken.getId())
                    .name(ssoToken.getFullName())
                    .withFirstName(ssoToken.getFirstName())
                    .withLastName(ssoToken.getLastName())
                    .withEMail(ssoToken.getEmail())
                    .addUserInfo(ssoToken.getUserInfo());

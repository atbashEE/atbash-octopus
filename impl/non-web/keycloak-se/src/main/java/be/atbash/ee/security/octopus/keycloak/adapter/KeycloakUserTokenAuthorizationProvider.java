 ...
 */
@ApplicationScoped
public class KeycloakUserTokenAuthorizationProvider implements TokenBasedAuthorizationInfoProvider {

    @Override
    public AuthorizationInfo getAuthorizationInfo(AuthorizationToken token) {
        if (token instanceof KeycloakUserToken) {
            KeycloakUserToken userToken = (KeycloakUserToken) token;
            AuthorizationInfoBuilder builder = new AuthorizationInfoBuilder();
            for (String group : userToken.getRoles()) {

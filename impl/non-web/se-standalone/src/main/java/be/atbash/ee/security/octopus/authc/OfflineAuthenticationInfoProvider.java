...
public class OfflineAuthenticationInfoProvider implements AuthenticationInfoProvider {
    ...
    public AuthenticationInfo getAuthenticationInfo(AuthenticationToken token) {
        ...
        if (token instanceof OfflineToken) {

            OfflineToken offlineToken = (OfflineToken) token;

            AuthenticationInfoBuilder builder = new AuthenticationInfoBuilder();
            builder.principalId(offlineToken.getId())
                    .userName(offlineToken.getSubject())
                    .name(offlineToken.getName());

...
public class MPJWTTokenBuilder {
    ...
    @Inject
    private MPCoreConfiguration mpCoreConfiguration;
    ...
    private void useDefaults() {
        if (!StringUtils.hasText(token.getIss())) {
            token.setIss(mpCoreConfiguration.getIssuer());
        }

        if (!StringUtils.hasText(token.getAud())) {
            token.setAud(mpCoreConfiguration.getAudience());
        }
        ...
        if (token.getExp() == null) {
            String expirationTime = mpCoreConfiguration.getExpirationTime();
